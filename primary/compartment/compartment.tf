...
locals {
  ...
  level_1 = [
    for k1, v1 in(var.compartments_configuration != null ? var.compartments_configuration.compartments : {}) : {
      ...
      enable_delete = var.compartments_configuration.enable_delete != null ? var.compartments_configuration.enable_delete : true
    }
  ]
  ...
  compartment_mapping = {
    # Map compartment names to their keys from compartments_configuration
    for k, v in var.compartments_configuration.compartments : k => k
  }
}
